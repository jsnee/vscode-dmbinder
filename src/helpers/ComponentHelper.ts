...
export namespace ComponentHelper {
    ...
    export async function promptGenerateComponent(item?: ITreeItem, includeAutogen: boolean = false): Promise<string | undefined> {
        ...
        if (componentUri) {
            ...
            if (selectedTemplate) {
                ...
                return wrapInAutogen(componentName, selectedTemplate.inferredFromComponent ? undefined : selectedTemplate.templateName, body);
            }
        }
        return;
    }
    ...
    export async function autogenerateComponentsForCurrentDocument(): Promise<void> {
        const doc = EditorHelper.getCurrentDocument();
        if (doc) {
            await autogenerateDocumentComponents(doc);
        }
    }
    ...
    function wrapInAutogen(componentName: string, templateName: string | undefined, body: string): string {
        if (templateName === undefined) {
            return `<dmb-autogen data-dmb-component="${componentName}">\n\n${body}\n</dmb-autogen>`;
        }
        return `<dmb-autogen data-dmb-component="${componentName}" data-dmb-template="${templateName}">\n\n${body}\n</dmb-autogen>`;
    }
}